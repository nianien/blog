...
# 返回 10 表示已复用当前项目服务，其他非零状态表示无法安全启动
check_server() {
  node scripts/server-status.cjs "$PWD" "$1" "$2"
}
...
cmd_dev() {
  check_env
  local status=0
  check_server 3000 dev || status=$?
  if [ "$status" -eq 10 ]; then return; fi
...
cmd_preview() {
  check_env
  local status=0
  check_server 8000 preview || status=$?
  if [ "$status" -eq 10 ]; then return; fi
  if [ "$status" -ne 0 ]; then return "$status"; fi
  # 自动构建（如果 out/ 不存在）
...
cmd_wx_preview() {
  if [ -z "$1" ]; then
    echo "用法: $0 wx:preview <md文件路径> [--serve] [--no-open]"
    exit 1
  fi
  check_env
  npx tsx scripts/wx/publish.ts --preview "$@"
}
...
微信公众号:
  ...
  wx:preview <md> [--serve] [--no-open]
                   生成微信排版预览 HTML；--serve 提供本机网页，--no-open 禁止自动打开
