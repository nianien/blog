...
Tencent 和 Fish 的分句粒度不同（逗号级 vs 标点分句），文本替换需要做对齐。核心策略是**按时间窗口匹配**：

1. 对于 Tencent 的每个句段（有精确的 start_ms / end_ms），找到时间窗口重叠的 Fish 句子
2. 如果 Fish 句子的文本与 Tencent 文本差异较大，用 Fish 的文本替换
3. 如果 Fish 在该时间窗口没有对应句子，保留 Tencent 原文

由于 Fish 前半段时间戳尚可（蛋烘糕场景在 0:33-0:54，Fish 此处偏差不大），时间窗口匹配在这个区间是可靠的。后半段两者共有的句子较少，影响有限。
...
综合准确率好，支持情感和语言检测，异步文件转写模式适合批量处理。唯一不足是说话人分离粒度较粗，且有小段漏识。
...
讯飞在方言核心词识别上与豆包并列第一，且覆盖率远高于豆包。当前分段过粗无法独立使用，但作为**方言文本校准源**（提供正确的方言词汇供 LLM 仲裁）价值很高。适合作为融合方案中的"第四参考源"。
