...
下文是用于解释设计原则的简化示例，不是某个项目的完整实现或性能报告；省略的业务类型、依赖和返回值需要在实际工程补齐。本文以一个**电商订单系统**为主线，将这些原则按目标维度分为四组，逐一拆解它们的工程含义和使用边界。文末的决策框架给出了原则冲突时的判断路径——因为真正的工程判断力，不在于记住所有原则，而在于感知原则之间的张力，并做出权衡。
...
第一种方案支持插件式规则引擎、动态加载、用户维度灰度——但示例只需要 3 个固定开关。若需要动态调整、灰度和审计，应补齐对应机制；需求只有静态开关时，一张映射表就能表达。

减少部件不是目的；让当前需求更容易理解和验证，才是简化是否有效的标准。

### YAGNI：代码是负债，不是资产

YAGNI（You Ain't Gonna Need It）针对未经确认的未来功能，不等于放弃改善当前设计。参见 [Martin Fowler 对 YAGNI 的解释](https://martinfowler.com/bliki/Yagni.html)。
...
**每一行代码都会产生维护成本。** 它要维护、要测试、要被后来的人理解。如果它解决的是一个不存在的问题，就是纯粹的负债。
...
用一组假设数据说明：若某个循环耗时 200μs，数据库查询耗时 800ms，循环仅占这两部分总耗时约 0.025%。即使将循环完全消除，总体改善也很小。先通过测量确定瓶颈，再决定优化对象；这组数字是计算示例，不是实测结果。
...
DRY 的定义强调同一知识应有单一、明确、权威的表示，而不只是代码外形一致 关键词是 knowledge（知识），不是 code（代码）。
...
订单系统中三个地方做手机号格式校验——用户注册、修改收货信息、绑定支付手机号。这三个场景的校验规则来自同一条业务规则："合法的中国大陆手机号格式"。这个正则只是示例中的格式初筛，不能证明号码已分配、可联系或归属本人。如果三处确实共享同一格式政策，政策变化时就应同步修改。这才是真正的知识重复，应该统一：
...
鉴权交给拦截器，参数校验交给注解，消息发送抽象成事件发布，减少变化传播。注解需要相应参数解析器支持；数据库与消息的一致性仍需事务边界或 Outbox 等机制，抽取方法不会自动解决双写问题。
...
交易模块只需要 `create` 和 `cancel`，报表模块只需要 `getStatistics` 和 `exportToExcel`，但两者都被迫依赖完整的 `OrderService` 接口。接口变化扩大依赖面，但不一定迫使未使用该方法的调用方修改或重编译；是否二进制兼容还取决于具体变更。
...
加一个通知渠道（站内信），需要修改 `create()`、`cancel()`、`refund()`、`complete()` 四个方法。**通知渠道（短信/邮件/站内信）和业务事件（创建/取消/退款/完成）是两个独立维度，但在这个设计里它们纠缠在一起。** 4 个事件与 3 个渠道共有 12 种组合；新增一个渠道通常需要检查 4 个事件处理点，组合数量不能直接当成此次修改点数量。
...
加站内信？实现一个 `InAppChannel`，注册到 channels 列表。改通知文案？只改 `toMessage()`。新增渠道主要落在渠道实现和注册处，减少逐个业务方法修改。契约、重试和渠道特有格式仍需适配，不能把开发成本当成算法复杂度承诺。

> **检验正交性的方法：问自己"如果我要替换 X，需要改多少个与 X 无关的文件？"** 据此检查不必要的传播；契约变更可能合理地影响多个调用方，文件数并非唯一标准。
...
一个架构评审中的简单判断标准：**检查依赖是否服务于同一业务职责，以及变更是否被不必要地传播。** 依赖数量可提示进一步审查，但没有“超过 7 个一定有问题”的通用阈值。
...
**依赖间的交互会增加需要理解和验证的组合。** 解决办法不是从 15 减到 14，而是重新审视职责划分——拆成 3-4 个更小的服务，每个只依赖 3-4 个接口。
...
这行代码把调用方和 Order、User、Address、City 四个类的内部结构绑死。后来地址模块重构，将 `City` 拆分为 `Region` 和 `District`——所有使用 `.getCity().getZipCode()` 的地方全部编译报错。如果订单、物流、发票、报表模块都这样访问，修改就会传播到多个模块。 一次结构重构变成了一场跨团队协调。
...
Order 内部怎么组织 User、Address、City 的关系是它自己的事。外部只知道"可以向 Order 要一个邮编"。当 City 拆分时，**只需修改 `Order.getShippingZipCode()` 的实现——1 个方法，1 个文件。** 调用方不必跟着内部对象结构改变。
...
此例只持有一个策略。若用户可以同时具备 VIP 和企业权益，还需一个组合策略，并定义折扣叠加、优先级和冲突规则。**用"有一个"代替"是一个"，用接口契约代替实现继承。**
...
当继承关系让你觉得不舒服时，通常意味着应该用组合（上一节的 `MembershipStrategy`）或重新建模（这个带独立宽高 setter 的可变模型可以改为共同实现 `Shape`；不能据此否定所有不可变矩形模型中的子类型关系）。
...
**依赖方向反转了**：不再是高层依赖低层，而是低层实现依赖高层定义的抽象接口。在新实现满足同样的事务、查询、一致性和错误语义时，业务代码才可能不改；MySQL 与 Elasticsearch 不能仅凭同名接口就视为可替换。
...
不要在第一版就建一个"通用支付网关框架"支持二十种支付方式的动态注册和热加载——但至少把支付逻辑藏在一个接口后面。定义一个接口的成本很低，但它留下的扩展空间很大。接口也有契约、版本和理解成本，错误抽象同样会制造缺陷。是否引入接口，要看它能否隔离当前已经存在的变化。
...
回到开头的元原则：**用最简单的方式解决当下的问题，同时不给下一个人制造麻烦。** 好的代码不是最聪明的代码，而是下一个人能看懂、能改动、能扩展而不心惊胆战的代码。代码在维护中会被反复阅读，固定的读写次数比例并不适用于所有项目。**为维护者编码**不是某一条原则，而是所有原则最终指向的方向。
