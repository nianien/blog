...
description: "编程原则是对抗软件复杂性的核心工具，但工具之间存在内在张力。本文以电商订单系统为主线，将八大编程原则按目标维度分为四组，逐一拆解其工程含义、典型误用与正确边界，并给出原则冲突时的决策框架。"
pubDate: "2025-10-18"
...
│ 统一知识 │ DRY                           │ 合并碰巧相似的代码      │
│ 划定边界 │ 关注分离 · 单一职责 · 正交性   │ 过度拆分 / 循环依赖     │
│ 管理依赖 │ 最小耦合 · 迪米特 · 组合 · OCP │ 链式穿透 / 滥用继承     │
└──────────┴───────────────────────────────┴─────────────────────────┘
```
...
顺带提一句里氏替换原则（LSP）：如果子类不能在所有场景下替代父类使用而不出问题，就不应该用继承。当继承关系让你觉得不舒服时，通常意味着应该用组合。
...
**一句话总结：依赖越少越好，知道得越少越好，继承能不用就不用，扩展靠新增不靠修改。**
