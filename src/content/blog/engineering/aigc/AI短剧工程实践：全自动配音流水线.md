...
短剧出海的核心难点不是翻译，是配音——原片只有一个 mp4，没有剧本没有角色表，单集 3-8 个说话人交替出现，一个剧 60-100 集，人工配音成本扛不住。

这篇记录的是我在实际项目中跑通的一套全自动配音流水线：ASR → 说话人分离 → 翻译 → TTS → 混音，单集成本 ¥0.3-0.5，支持批量生产。不是模型评测，是工程落地。
...
把 ASR、翻译、TTS 串成一条**可运营的流水线**，难点不在模型本身：
...
这是整条流水线中**技术复杂度最高的环节**——需要处理多角色声线分配、语速适配、情绪控制、缓存复用。本节聚焦**出海配音场景下的商业 TTS API 选型和工程集成**；如果你关注的是角色声纹克隆（用 30 秒音频训练一个专属声线），参见[第 3 篇《角色生产流水线》第四章](/blog/engineering/aigc/AI短剧工程实践：角色生产流水线)。
...
**一个重要判断**：端到端模型在单人、简单场景下已经很好，但在多角色、多情绪、严格时间轴约束的短剧场景下，串行管线（ASR→MT→TTS）的可控性和可干预性仍然是不可替代的优势。当端到端模型的质量和可控性达到串行管线的水平时，才是架构切换的时机。
...
这套流水线的设计思路不限于配音场景。任何需要把多个 AI 能力串联成生产系统的场景——AI 视频生成、AI 内容审核、AI 数据标注——都面临相同的核心问题：**模型只是零件，真正的挑战在于如何把零件组装成可靠的机器。**

几条可复用的原则：

- **SSOT 优于 pipeline 内传参**：中间产物显式落盘、有 schema 版本，比内存传递更可调试、更可恢复
