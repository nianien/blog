...
前两篇完成了技术选型和Pipeline搭建。但Pipeline还不够，有Pipeline不等于能做AI短剧产品。
...
模型（Stable Diffusion、视频生成模型、TTS引擎）只负责"画画"和"说话"。决定画什么、怎么画、怎么拼接、角色状态延续、成本控制这些都是应用层的工作。
...
没有导演只有演员，每一帧都不一样，电影拍不出来。很多团队Demo效果惊艳但产品做不出来，因为只搭了Pipeline（雇了演员），没有建应用层系统（没有导演团队）。
...
Pipeline级别，角色可能就是`hero_lora_v2.safetensors`。但在产品级别，角色是一个**多模态实体**，有脸、声音、性格、穿衣风格、行为习惯。这些信息必须结构化存储，让系统在每次生成时做出正确决策。
...
1. **视觉和音频资产解耦**：视觉用LoRA+参考图，音频用声纹+参考音频。两者通过`character_id`关联但独立管理。更换声音不影响外观。

2. **多版本管理**：LoRA迭代训练，声纹持续优化。`version`和`last_trained`保证系统使用最新资产，同时支持回滚。

3. **衣柜系统**：角色换装很常见。预定义几套服装标签，生成时通过prompt切换。换装导致一致性下降时，可为特定服装训练专属LoRA。

4. **质量评分**：`quality_score`是角色资产可用性的量化评估（通过自动化测试生成多角度/多表情图片评分）。低于阈值需重新训练。

5. **Face Swap源图**：`face_swap_source`存储Face Swap后处理用的高质量正脸照片，与IP-Adapter参考图分开管理。Face Swap需高清正脸，IP-Adapter需多角度参考。

6. **负向提示词和风格标签**：`negative_prompt`和`style_tags`是角色级别的生成约束。不同角色可能需不同的负向提示词和风格偏好（写实vs轻微美化），固化在注册表中避免每次手动设置。

7. **外观特征**：`age_appearance`和`skin_tone`给LLM剧本转换和prompt生成使用，确保自动生成的prompt包含正确的年龄和肤色。

### 角色资产的生命周期

![角色资产生命周期](/images/blog/aigc/char-lifecycle.svg)

角色资产经历四阶段：**创建**（收集参考图、训练LoRA、录制声纹）→ **验证**（用标准测试集跑一致性评分，达标标记为production-ready）→ **运行**（在生产Pipeline中引用，监控一致性分数）→ **迭代/退役**（模型升级效果漂移时重新训练，或角色下线）。

**监控漂移**容易被忽略。基础模型更新或Pipeline参数调整，同一LoRA效果可能变化。做法是维护标准测试Prompt集合（覆盖正脸、侧脸、不同光照），每次模型或参数变更后自动跑一遍，用ArcFace余弦相似度打分（阈值0.75），低于阈值触发重新训练告警。

### 多角色场景的冲突处理

两个角色同时出现时，两个LoRA权重可能互相干扰，角色A的特征"泄漏"到角色B。

**工程解决方案**：

1. **区域控制**：用Regional Prompting或Attention Couple技术分区，每个区域只激活对应角色LoRA。ComfyUI有成熟的区域提示词插件。

2. **分层合成**：分别生成每个角色（透明背景），再合成到同一场景。工程复杂但一致性最好。

3. **优先级策略**：主角用LoRA+IP-Adapter保证一致性，配角用InstantID快速生成。
...
核心工作是**将结构化状态翻译为Pipeline可理解的条件组合**：
...
互动短剧（用户可做选择影响剧情），状态管理需支持**分支和回溯**。

![状态分支与回溯](/images/blog/aigc/branching-state.svg)

主线剧情沿时间轴推进，每个决策点（如"选A还是B"）分叉出子分支，每条分支携带独立的世界状态副本。用户回溯时，系统从对应决策点的快照恢复状态，不是回退所有计算。

工程用**状态快照**实现：每个决策点保存完整世界状态，用户选择不同分支时从快照恢复，本质和游戏存档相同。快照粒度选择：保存所有字段（简单但存储大）还是只保存diff（节省空间但恢复慢），取决于状态对象大小和决策点密度。
...
Pipeline需要结构化的生成指令。**剧本中间件**就是这个翻译层，把自然语言剧本转化为机器可执行的任务序列。
...
**准确率**：当前LLM对简单对话场景的分镜拆解准确率约80-90%，复杂动作场景（打斗、追逐）约50-60%。人工审核仍需要，但工作量从"从零编写"降低到"审核修改"。提高准确率的关键是约束输出空间，词表越小、Schema越严格、few-shot示例越多，输出越可靠。
...
一部短剧有100-200个镜头，调度系统需管理所有镜头的生成、排队、重试和拼接。

![多模态任务调度](/images/blog/aigc/task-scheduling.svg)

核心是一个**任务队列 + 依赖图**：每个镜头拆解为多个子任务（视觉生成、音频合成、Lip-sync对齐、质量检测），子任务有依赖关系（音频必在Lip-sync前完成）。调度器按依赖图拓扑排序分配GPU/CPU资源，同一层级无依赖任务并行执行。子任务失败时只需重试该任务及其下游。最终镜头按剧本顺序拼接。
...
**关键洞察**：算力成本已很低（即使含重试也不到¥200/集）。真正成本大头是**人工审核和修帧**。当前AI生成首次可用率约50-70%，经QA自动重试后可提升到80-90%，剩余需人工介入。降低人工成本关键不是更好的模型，而是更好的应用层系统。QA自动检测+自动重试可将需人工处理的case从40%降到10-20%。
...
缓存可降低实际需要生成的工作量30-50%。

### 错误处理与重试策略

批量生成100-200个镜头时，失败是常态。健壮的调度系统必须包含完整的错误处理链。
...
AI生成内容存在不可控性，即使prompt正常，扩散模型也可能生成不当内容（NSFW、暴力、文化敏感元素）。面向用户发布前必须有审核环节。
...
LLM生成的结构化剧本JSON不能直接信任，格式可能正确但内容可能有逻辑错误。进入Pipeline前需程序化验证。
...
AI短剧项目可能持续数周到数月，角色LoRA迭代、基础模型更换、Pipeline参数调优。不做版本管理，很容易出现"第5集和第1集的角色看起来不像同一个人"。
...
游戏行业的人读到这里应该发现了，上面这套架构和游戏引擎结构惊人地相似。
...
这不是巧合。AI短剧系统和游戏引擎面对同一个问题：在有状态的虚拟世界中管理多个实体的行为和表现，并实时或近实时渲染输出。

区别只是渲染器不同——游戏用光栅化/光线追踪，AI短剧用扩散模型。但上层的架构逻辑几乎完全相同。

这是为什么很多做得好的AI短剧团队，核心架构师都有游戏引擎背景。
...
大多数团队在阶段一到阶段二之间。能做到阶段三的，就是下一个"AI内容平台"。
...
同一角色在不同语种下使用不同声纹和TTS引擎，目标是保持音色特征的跨语种一致性，听起来像"同一个人在说不同语言"。

当前跨语言声纹克隆的相似度通常只有同语言的60-70%，中文声纹合成英文语音时音色会有明显偏移。应对策略：
- 为目标语种单独录制参考音频（哪怕只有30秒），效果远好于纯跨语言克隆
- 同语言声纹克隆可以达到90%+的相似度，跨语言时降低验收标准
- CosyVoice在跨语言场景下表现相对最好，出海场景建议优先测试

配合Lip-sync的口型重绘能力，可实现"一次拍摄，多语种发行"，视觉资产不变，只替换音频和口型。这是AI短剧出海的核心工程优势。
