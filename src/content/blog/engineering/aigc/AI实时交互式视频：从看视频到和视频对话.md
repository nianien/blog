...
> 视频从"播放媒介"变成"交互界面"。用户不再被动观看，而是通过每一句话、每个动作驱动内容的实时生成。
...
![内容形态演进](/images/blog/aigc/content-evolution.svg)

前三次都是"创作者出内容，用户消费内容"。AI 交互式视频不同——用户每次互动，系统实时生成下一段视频。这改变了整个模式。
...
> 关于预制短剧场景下的世界状态工程实现（实体注册表、状态持久化、分支管理），参见[第 4 篇《从视频生成到角色引擎》第三章](/blog/engineering/aigc/AI短剧工程实践：从视频生成到角色引擎)。本节聚焦实时交互场景的特有挑战。
...
一个容易被忽视的维度：**用户愿不愿意主动互动？**

很多用户看视频是被动接收，切换到主动说话会有认知成本。这对交互设计有特殊要求：
...
当前 AI 产品主要通过聊天框交互。但视频有更高的信息密度（视觉 + 听觉 + 文字）和更低的认知门槛。一段会响应你的视频，可能比文字对话更自然。
...
对于互动视频场景，关键评估维度不是画面质量（各家差异不大），而是**延迟**和 **API 灵活度**。HeyGen 和 D-ID 都提供了 Streaming API，支持文本输入到视频输出的低延迟流式生成，可以直接接入对话系统。Synthesia 偏向离线批量生成，实时性较弱。选型时建议先用 D-ID 快速验证交互体验，跑通后再根据画质和成本需求切换到 HeyGen 或自建方案。
...
**前端播放层**负责视频渲染、用户输入捕获和交互 UI。核心挑战是视频片段的无缝切换——预加载下一段视频、用过渡动画遮盖生成延迟。技术选型上 Web 端用 HLS.js + Canvas 叠加交互元素，移动端用原生播放器 + WebView 混合方案。

**业务逻辑层**是系统的"大脑"：接收用户输入 → 调用 LLM 理解意图 → 查询世界状态 → 决定下一段内容 → 调度 AI 服务生成（或从缓存取预制片段）→ 返回视频流。Session 管理和状态持久化是这一层的核心，用 Redis 做热状态缓存、PostgreSQL 做冷存储。

**AI 服务层**封装所有生成能力（视频生成、TTS、数字人、LLM）为独立微服务，通过消息队列（如 RabbitMQ 或 Redis Streams）异步调用。这层的设计原则是**可替换**——每个 AI 能力都是一个接口，底层供应商随时可以切换。
...
> AI 互动视频改变的不只是一个功能，而是内容与用户的关系。技术成熟只是前提。用户习惯的迁移、商业模型、监管框架，每一个都是关卡。

核心结论：混合架构（预制内容 + 局部 AI 生成）是近期最务实的方案。真正的壁垒不在视频生成，而在内容编排和交互体验。B2B 领域（培训、营销）最可能先跑通，C 端娱乐需要成本继续下降。
