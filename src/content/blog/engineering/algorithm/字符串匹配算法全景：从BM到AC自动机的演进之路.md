...
字符串匹配问题的形式化定义如下：给定文本串 T（长度为 n）和非空模式串 P（长度为 m），在 T 中查找 P 出现的所有位置，包括重叠匹配。空模式的语义需由接口单独约定。
...
- **特定方向优化**：Sunday（比较顺序可选，跳转利用窗口后一位字符）
- **基于哈希**：Rabin-Karp（先比较哈希，精确匹配仍需核验候选）
...
1. **字符 c 不在模式串中出现**：移动 j+1 位，j 为失配位置的模式下标；只有 j=m-1 时才等于整个窗口长度。
...
模式串: D A B C A B
好后缀:         A B
另一次 AB 位于下标 1..2，前驱 D 不同于好后缀前驱 C
将其右移 3 位，与文本中的已匹配 AB 对齐
```
...
| 场景 | 时间复杂度 | 说明 |
|------|-----------|------|
| 最好情况 | O(n / m) | 右端首次比较失配后跳过 m 位，另计预处理 |
| 最坏情况 | O(n * m) | 退化为朴素匹配，如 T = "AAAA..."、P = "AAA" |
| 平均情况 | 在相应文本分布下可亚线性 | 实际文本中表现优异，通常远快于 O(n) |

上表的最坏情况针对未采用 Galil 等优化、报告全部出现位置的基本版本。

BM 算法在处理自然语言文本时可能较高效，因为坏字符规则在字符集较大时跳转距离更长。
...
BARBER 中 B 在位置 0 和 3，E 在位置 4；排除末尾位置 5 的 R 后，B 的 shift 为 2，E 为 1。

Horspool 算法的最坏时间复杂度仍为 O(n * m)，但在实际应用中的平均性能与 BM 非常接近，且实现简洁得多，是许多工程场景下的首选。

### Sunday 算法：面向实际场景的进一步优化

Sunday 算法由 Daniel M. Sunday 于 1990 年提出，从设计理念上看，它走了一条与 BM/Horspool 不同的路径：**跳转参考窗口外下一字符**（字符比较顺序可以自行选择），但在失配时利用了一个独特的观察。
...
| 步骤 | 文本 T = ABCXEABCD，模式 P = ABCD |
| --- | --- |
| 窗口从 0 开始 | T[3]=X 与 P[3]=D 失配 |
| 查看窗口外字符 | T[4]=E，不在模式中，移动 m+1=5 位 |
| 窗口从 5 开始 | T[5..8]=ABCD，匹配成功 |

窗口已经到达文本末尾时，不再读取 T[i+m]，避免越界。
...
Sunday 算法的优势在于最大跳转距离为 m + 1（比 BM 的 m 还多一位），且实现极其简洁。在短模式串和字符集较大的场景下（如英文文本搜索），Sunday 可能表现较好，仍需与目标数据和实现比较。但在最坏情况下（如二进制文本中搜索重复模式），其复杂度同样退化为 O(n * m)。
...
| Sunday | 可选 | 窗口下一位字符 | O(\|Sigma\| + m) | 最低 |
...
该优化只用于合法字符下标 j、k，并应保留未优化的 next[m] 作为完整匹配后的回退长度；不能在 j=m 时访问 P[m]。它不改变渐进复杂度。
...
当 h(T[i..i+m-1]) == h(P) 时，存在两种可能：真正匹配，或哈希冲突。因此必须进行逐字符验证。选择合适的素数 q 可以降低冲突概率。在理论分析中，冲突概率需要结合随机化方式、模式长度与哈希族分析，不能仅凭固定模数大小给出通用的 O(1/q) 保证。

**复杂度分析**：

| 场景 | 时间复杂度 | 说明 |
|------|-----------|------|
| 候选稀少时 | O(n + m) | 另计真匹配和哈希冲突候选的逐字验证 |
| 最坏情况 | O(n * m) | 所有窗口哈希均冲突（如 q 选择不当） |

#### RK 的独特优势

RK 算法在单模式匹配中并不比 BM/KMP 更优，但它有一个独特的应用场景：**多模式串的同时匹配**。当需要在文本中同时搜索 k 个等长的模式串时，可以将所有模式串的哈希值存入哈希表，每次窗口滑动后查表比较，哈希预处理和扫描的期望成本约为 O(n + k*m)，还要计入候选验证与输出。重复模式或大量匹配会增加这部分成本。此外，RK 算法天然适合二维模式匹配（在矩阵中搜索子矩阵）等扩展场景。
...
AC_Match(T):
    ...
    for i = 0 to n - 1:
        ...
        // 构建阶段已经合并 failure 的 output，不能再次沿链重复报告
        for each pattern in output(state):
            report pattern at start position i - length(pattern) + 1
```

文本串的每个字符只被读取一次，指针始终向前。当前版本直接枚举已经合并的 output，避免重复报告。也可只保存本节点模式并使用输出链接枚举后缀匹配，但不能同时采用两种方式。
...
| 构建 failure 与转移 | 取决于表示；完整转移表可按 O(L·\|Sigma\|) 构建 | 逐条回溯版本不能仅凭 BFS 就断言总计 O(L) |
| 匹配 | O(n + z) | n 为文本长度，z 为匹配结果总数 |
| 空间 | 转移结构另加 output 存储 | 显式复制所有后缀输出可能增大空间，可改用输出链接 |
...
| AC 自动机 | 依转移与输出表示 | O(n + z) | O(n + z)，假设转移查找 O(1) | 转移结构 + 输出结构 | 多模式匹配 |
...
**需要严格最坏复杂度保证**：KMP 算法。在安全相关场景中（如正则表达式引擎的实现），算法的最坏性能不可接受为 O(n * m)，KMP 是一种简单选择，也有其他具备线性保证的精确匹配算法；KMP 本身不负责一般正则表达式语义。

**多个等长模式串**：Rabin-Karp 算法。通过哈希表存储所有模式串的哈希值，一次滚动扫描即可完成。

**大规模多模式匹配**：AC 自动机。模式串数量从几十到数万，AC 自动机的匹配时间与模式串数量无关（仅与文本长度和匹配结果数相关），是值得优先评估的方案之一。
...
- **预处理开销**：如果文本很短或匹配只执行一次，预处理的固定开销可能超过它带来的收益，朴素算法可能更合适。
