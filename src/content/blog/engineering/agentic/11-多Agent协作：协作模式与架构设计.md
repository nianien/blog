...
![一个"全能" Agent 的 Context 分配](/images/blog/agentic-11/01-context-allocation.svg)
...
这个问题在多 Agent 系统中不会消失，反而会以乘法放大。假设单个 Agent 的任务成功率是 80%，两个串联 Agent 的系统成功率不是 80%，而是 80% × 80% = **64%**；三个串联就是 80%³ ≈ **51%**——刚过一半。这就是为什么多 Agent 架构必须在每个节点引入校验和容错机制，而不是简单地把 Agent 串起来。

**天花板四：并行度限制**

单 Agent 的执行是串行的——一次 LLM 调用，等待结果，再进行下一次。如果一个任务可以分解为三个独立子任务（比如同时搜索三个数据源），单 Agent 只能顺序执行，浪费了大量时间。

![Single-Agent 串行执行 vs Multi-Agent 并行执行](/images/blog/agentic-11/02-serial-vs-parallel.svg)
...
![Supervisor-Worker 协作模式](/images/blog/agentic-11/03-supervisor-worker.svg)
...
![Peer-to-Peer 协作模式](/images/blog/agentic-11/04-peer-to-peer.svg)
...
![Pipeline 流水线架构](/images/blog/agentic-11/05-pipeline.svg)
...
![Dynamic Routing 动态路由架构](/images/blog/agentic-11/06-dynamic-routing.svg)
...
![共享黑板 Blackboard Pattern](/images/blog/agentic-11/07-shared-blackboard.svg)
...
![消息传递 Message Passing Pattern](/images/blog/agentic-11/08-message-passing.svg)
...
![发布/订阅 Event Bus Pattern](/images/blog/agentic-11/14-event-bus.svg)
...
![状态管理方案：共享状态 vs 独立状态](/images/blog/agentic-11/09-state-management.svg)
...
![Worker 失败处理决策树](/images/blog/agentic-11/10-worker-failure-recovery.svg)
...
![死锁场景示例](/images/blog/agentic-11/11-deadlock-scenario.svg)
...
![成本模型对比：Single-Agent vs Multi-Agent](/images/blog/agentic-11/12-cost-comparison.svg)

### 7.2 什么时候 Multi-Agent 的收益大于成本

不是所有场景都值得用 Multi-Agent。一个简单的决策框架：

![Multi-Agent 决策矩阵：何时使用 Multi-Agent](/images/blog/agentic-11/13-multi-agent-decision.svg)
...
![Supervisor-Worker 成本分析](/images/blog/agentic-11/15-cost-supervisor-worker.svg)

#### 模式二：Peer-to-Peer

![Peer-to-Peer 成本分析](/images/blog/agentic-11/16-cost-peer-to-peer.svg)

#### 模式三：Pipeline

![Pipeline 成本分析](/images/blog/agentic-11/17-cost-pipeline.svg)

#### 模式四：Dynamic Routing

![Dynamic Routing 成本分析](/images/blog/agentic-11/18-cost-dynamic-routing.svg)
