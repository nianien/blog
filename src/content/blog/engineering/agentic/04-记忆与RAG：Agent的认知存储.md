...
上一篇的 Runtime 能推进状态机，但状态如果只停留在当前进程和完整对话历史里，任务一长就会被 Token 和噪声拖垮。
...
为避免与自主性等级 A1-A4、学习时间尺度 T1-T3 混淆，本系列跨章引用记忆层时统一使用 **M1-M4**；本篇既有表格中的 L1-L4 均表示 Memory Layer，不表示自主性。
...
记忆解决的是"过去什么信息值得进入当前上下文"，复杂任务还要决定"未来先做什么、走哪条路径、错了如何回头"；下一篇进入规划、推理与反思。
