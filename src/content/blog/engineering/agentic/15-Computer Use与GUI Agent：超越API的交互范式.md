...
![Computer Use Agent 主循环](/images/blog/agentic-15/computer-use-loop.svg)
...
以用户请求"帮我查一下上月的销售数据，然后在报销系统里提交差旅费用"为例：

| 子任务 | 交互方式 | 执行方式 | 耗时 | 成本 |
|-------|---------|---------|------|------|
| 查询销售数据 | Tool Calling（有 API） | 调用 `sales_api.query(month="2026-03")` | 200ms | ~$0.001 |
| 提交差旅报销 | Computer Use（报销系统无 API） | 截图→登录→导航→填表→上传附件→提交 | 60-120s | ~$0.50-1.00 |
