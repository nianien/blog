...
上一篇把多步任务固化成显式工作流；本篇只处理其中一种升级：当一个节点不能再用普通函数或单次 LLM 调用表达，而需要自己的上下文、工具和控制循环时，它才成为独立 Agent。
...
协作模式确定后还要落到具体实现：状态放在哪、检查点如何恢复、工具如何接入、Agent 如何互通。下一篇区分框架与协议，避免把开发抽象和生态互操作混成一个选型问题。
