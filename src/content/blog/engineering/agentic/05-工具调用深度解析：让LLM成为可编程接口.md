...
把所有工具全部传递给 LLM。适用场景：工具少于 10 个。简单暴力，无额外开销。

**策略二：语义过滤**

用 Embedding 计算用户输入与工具描述的语义相似度，只传递 Top-K 最相关的工具：用户输入和工具描述分别做 Embedding，通过向量相似度检索出候选工具再传给 LLM。缺点是可能漏掉正确工具。

**策略三：两阶段选择**

分两阶段筛选：阶段 1 将所有工具名 + 简短描述传给 LLM，由 LLM 选出 3-5 个候选；阶段 2 只将候选工具的完整定义传给 LLM，执行 Tool Call。
