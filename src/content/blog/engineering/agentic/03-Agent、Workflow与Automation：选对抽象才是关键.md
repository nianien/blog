...
选择 Workflow 框架
       │
       ├─→ 是否是 AWS Only 环境？
       │     ├─ Yes → Step Functions（简单，无运维）
       │     └─ No ↓
       │
       ├─→ 工作流长度？
       │     ├─ 分钟级/小时级 → Airflow 或 Prefect
       │     ├─ 天级/跨周期 → Temporal
       │     └─ 秒级 → 考虑是否真的需要 Workflow
       │
       ├─→ 团队技术栈？
       │     ├─ Python-first → Prefect > Airflow
       │     └─ 多语言 → Temporal (Go, Java) 或 Airflow
       │
       └─→ 运维能力？
             ├─ 低，不想自建 → Step Functions
             ├─ 中等 → Airflow（生态成熟）
             └─ 高，对可靠性要求极高 → Temporal
```
...
>
> - 上一篇：[02 | 从Prompt到Agent：为什么LLM本身不是Agent](/engineering/agentic/02-从Prompt到Agent：为什么LLM本身不是Agent)
> - 下一篇：[04 | Agent控制循环：运行时的核心抽象](/engineering/agentic/04-Agent控制循环：运行时的核心抽象)
