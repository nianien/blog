...
pubDate: "2026-04-28"
description: "当 Agent 像人类一样看屏幕、点鼠标、敲键盘，一种新的交互范式出现了。本文给出 Computer Use 的视觉-动作循环伪代码、action schema、定位策略（坐标 vs Set-of-Mark）、Tool Calling 与 Computer Use 的混合路由，以及为什么 OSWorld 72% 意味着失败率距离高风险生产要求仍有 1-2 个数量级。"
...
当目标系统没有 API、Tool Calling 这条精准路径走不通时，Agent 必须像人类一样——看屏幕、点鼠标、敲键盘。这就是 Computer Use。从 2024-10 Anthropic 首发到现在，OSWorld 成功率从 15% 飙升到 72%+，看似很快接近生产可用。但 benchmark 上的"72% 超人类基线"并不等于生产可用：若高风险任务要求失败率降到 1% 甚至 0.1%，当前约 28% 的失败率仍需下降 1-2 个数量级。任务边界清晰性、视觉注入风险、单次 token 成本是 Tool Calling 的 100-1000 倍，都会继续放大这段距离。

上一篇讨论的是结构化工具里的可信边界：参数有 schema、工具有权限、关键操作能审批。Computer Use 把这个边界放大到整个 GUI 环境——屏幕上的每个按钮、弹窗、隐藏文本都可能成为输入或动作目标，所以它必须在可信架构之后讨论，而不能只当成"更强的自动化"。
...
更隐蔽的盲区是 **benchmark 的任务边界清晰性**。OSWorld 上的任务有明确的成功判定（文件是否被创建、特定值是否被修改），但真实生产任务的成功标准往往模糊——"帮我整理一下这周的工作汇报"在 benchmark 上不存在，但在生产环境里很常见。这类任务的成功率，远比 benchmark 数字低。所以**看到 72% 超过人类基线，不要立刻得出 Computer Use 已经成熟的结论**——若高风险生产目标把失败率压到 1% 甚至 0.1%，当前约 28% 的失败率仍需下降 1-2 个数量级。
...
**Computer Use 提升 5% 和 Tool Calling 提升 5% 的工程价值完全不同**。Tool Calling 从 95% 到 99% 可能让低风险、可恢复的操作从基本可用变成可规模化；Computer Use 从 70% 到 75% 仍然意味着每四次任务就有一次失败。**生产决策的关键不是统一跨过某个成功率阈值，而是失败成本、可恢复性和人工兜底能否共同把残余风险压到可接受范围**——这是 Computer Use 仍然需要兜底机制的根本原因。
...
2024-10 到 2026 初，OSWorld 成功率从 15% 飙升到 72%+。按这个趋势，特定垂直场景（Web 表单填写、特定 SaaS 操作、标准化流程）的模型成功率有望 1-2 年内达到 95%+。这对低风险、可重试、有幂等保护的任务可能已经可用；资金、删除和跨域操作仍需要更高的端到端可靠性与 HITL。
...
OSWorld 从 15% 到 72%+ 的提升让人兴奋，但要冷静地看到两个事实：benchmark 任务边界清晰、生产任务边界模糊；若目标失败率是 1% 到 0.1%，当前失败率仍需下降约 1-2 个数量级。且 Computer Use 的 5% 提升和 Tool Calling 的 5% 提升工程价值完全不同——Tool Calling 95% 到 99% 可能跨过特定低风险场景的可用门槛，Computer Use 70% 到 75% 仍然意味着每四次任务就有一次失败。
...
这里的"自动学习并固化"不是 Computer Use 自带能力，而是独立的经验验证与策略发布闭环。下一篇把这种闭环与会话内适应、跨会话经验沉淀一起纳入学习时间尺度。
