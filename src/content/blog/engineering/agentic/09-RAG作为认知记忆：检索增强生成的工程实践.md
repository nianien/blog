...
![RAG 六层架构](/images/blog/agentic-09/rag-six-layer.svg)
...
原始文档被切分为两级：**大 chunk**（parent） 保留完整的一节内容（~2000 tokens），其下包含多个 **小 chunk**（child）（每段 ~200 tokens）。检索时用小 chunk 做向量匹配，命中后返回对应的大 chunk 给模型。
...
存储时，给每个 chunk 记录其前后邻居的 ID（prev_id / next_id），形成 chunk[n-1] ← chunk[n] → chunk[n+1] 的链式关联。检索时，命中 chunk[n] 后自动拼接前后邻居 chunk[n-1] + chunk[n] + chunk[n+1]，再送入 LLM。
...
用户提问后，Agent 先做**意图路由**：知识问答走 RAG Pipeline，数据查询走 SQL / API Tool，操作执行走 Function Calling，混合类型则先检索上下文再调工具。路由完成后进入**上下文组装**（RAG 结果 + Tool 结果 + 对话历史），最后**生成回答**。
