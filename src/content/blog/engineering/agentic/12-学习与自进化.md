...
title: "学习与自进化"
pubDate: "2026-05-19"
description: "静态 Agent 的天花板是重复犯错。本文给出运行时学习的三个时间尺度（T1 会话内适应、T2 跨会话经验沉淀、T3 行为策略进化）的具体 schema、经验回放与 Prompt 自优化的伪代码、动态 Few-shot 的检索逻辑，以及一条不可越过的红线——学习不能突破安全策略、不能升级权限、不能扩展行为空间。"
...
上一篇把重复 GUI 操作沉淀为结构化工具作为一个可能方向，但这种沉淀需要独立的经验验证、评估、审批与发布闭环。
...
为避免与自主性等级 A1-A4、记忆层 M1-M4 混淆，三个学习时间尺度统一记为 **T1-T3**；本篇后文原有的 L1-L3 均表示 Learning Timescale。
...
**T1** 依赖 LLM 的 In-context Learning——用户在对话中纠正"不要这么正式"、"用表格输出"，Agent 在同一会话中调整。它几乎没有额外基础设施成本，但会增加上下文 Token；会话结束后持久性归零，下一个会话开始时所有 T1 调整都消失。
...
当经验库、评估流水线、策略版本、Guardrails 和回滚机制不再只服务一个 Agent，它们就不该继续散落在各项目内部。下一篇把前十二篇能力收束成组织级 Agent 基础架构平台。
