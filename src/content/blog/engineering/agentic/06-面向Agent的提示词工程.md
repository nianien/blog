...
- `prompts/router/` — 路由 Prompt，包含 v1.0（初始）、v1.1（修复低 confidence 行为）、v2.0（新增 confidence 字段）、changelog.md
- `prompts/planner/` — 规划 Prompt，v1.0、v1.1
- `prompts/executor/` — 执行 Prompt，v1.0
- `prompts/reflector/` — 反思 Prompt，v1.0
...
假设一个 Agent 进行了 10 轮多步骤对话，初始状态为：System Prompt 2000 tokens，Turn 1-3 共 3000 tokens，Turn 4-7 共 8000 tokens，Turn 8-10 共 5000 tokens，合计 18,000 tokens（超出 8K 预算）。目标是压缩到 8,000 tokens（可用 6,000 for messages）。
...
| 指标 | GPT-4 | Claude | Gemini | Llama-2 |
|------|-------|--------|--------|---------|
| JSON 遵循度 | 99.2% | 98.7% | 97.1% | 94.3% |
| 长约束列表遵循 | 94% | 96% | 91% | 78% |
| System 优先级 | 强 | 极强 | 中等 | 弱 |
| Few-shot 敏感度 | 中 | 低 | 高 | 高 |
| 幻觉率 | 2.1% | 1.3% | 3.2% | 5.8% |
...
> - 实际：`Let me search for weather information...` 或未闭合的 JSON
