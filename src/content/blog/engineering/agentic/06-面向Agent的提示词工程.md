...
需要压缩 context?
    ...
    │       ├─ 是否有 Token 预算用于压缩？
    │       │   │
    │       │   ├─ 有（可以多花 300-500 token）
    │       │   │   └─→ 用摘要（Summarization）
    │       │   │       场景：复杂长期任务、需要追溯历史
    │       │   │       例：多天的任务规划、持续学习的 Agent
    │       │   │
    │       │   └─ 没有，Token 很紧张
    │       │       └─→ 用选择性保留（Selective Retention）
    │       │           场景：超低延迟、Token 极限场景
    │       │           例：边缘设备、成本敏感的生产系统
    │       │
    │       └─ 历史中有关键决策节点吗？
...
>
> - 上一篇：[05 | Tool Calling Deep Dive](/engineering/agentic/05-工具调用深度解析：让LLM成为可编程接口)
> - 下一篇：[07 | Agent Runtime from Scratch](/engineering/agentic/07-从零构建Agent运行时)
