...
至于每个组件具体怎么工作、A1 到 A4 不同自主性等级如何切分、成本结构里 token 为什么有时是小头、有时又是平台的核心计量单位——这是后续专题的范围，不在这张地图里展开。
...
对应到本系列的阅读路径，可以按"先单体、再组合、再生产、最后平台"来走：

| 阶段 | 文章 | 依赖关系 |
|------|------|---------|
| 定义边界 | 01 知识地图、02 本质与形态 | 先定义 Agent 解决什么、不该解决什么 |
| 单 Agent 底座 | 03 运行时、04 记忆与 RAG、05 规划推理反思 | 03 是运行时底座，04/05 分别补状态与认知能力 |
| 组合能力 | 06 工作流编排、07 多 Agent 协作 | 先把单 Agent 内部拓扑显式化，再把拓扑扩展到多 Agent 间 |
| 生态与生产 | 08 框架协议、09 生产级 Agent、10 可信 Agent | 框架解决开发效率，生产层和可信层解决上线后的可控性 |
| 进阶与规模化 | 11 Computer Use、12 学习自进化、13 基础架构平台 | GUI 交互和学习能力最终沉淀为平台能力 |

模型层不是单独一篇，而是横穿 05、08、09、13：05 讨论推理模型对规划架构的影响，08 讨论 SDK 与模型厂商抽象，09 讨论模型分层与成本路由，13 把这些能力收敛进 LLM Gateway。

为避免不同维度都叫"L1/L2/L3"，本系列统一使用三组前缀：**A1-A4** 表示 Autonomy Level（自主性等级），**M1-M4** 表示 Memory Layer（记忆层），**T1-T3** 表示 Learning Timescale（学习时间尺度）。日志冷热等级只在可观测性专题内部使用 `Log-L1/Log-L2/Log-L3`。后文单独出现"A3"时只表示任务驱动型 Agent，"M3"只表示情景记忆，"T3"只表示跨版本策略进化。
