...
上一篇把安全列为生产最低门槛，本篇只展开这条横切能力：检测可以依赖规则、分类器或 LLM，但拦截点和处置策略必须由系统强制执行。
...
可信 Agent 的目标不是"让 LLM 永远不犯错"——这做不到。目标是**构建多层防护，让错误在造成不可逆后果前被拦截**。Guardrails 是系统强制执行的自动防护层，判定手段既可以是确定性规则，也可以是概率模型；HITL 是人工兜底层。一个生产 Agent 的可信架构通常长这样：
...
平台化之后尤其要记住一个关键点：**HITL 不可扩展**。人类操作员可以管几百次审批，管不了几百万次。这就有了一个根本权衡：
...
可信边界确定之后，下一类问题是：如果目标系统没有 API、没有 MCP Server，Agent 还能不能行动？这会把安全边界从结构化工具参数扩展到整个屏幕环境，也就是 Computer Use 与 GUI Agent 的问题。
