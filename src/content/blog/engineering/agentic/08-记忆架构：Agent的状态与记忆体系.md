...
问题就变成了：怎么为一个 stateless 的 LLM 构建一套 stateful 的记忆体系，让 Agent 在有限的 Context Window 内获得"无限"的记忆能力？
...
上面两种方案只是消息级别的压缩。如果把视角拉高到整个记忆系统，Agent 的记忆压缩可以归纳为五类方法，每类解决不同层级的问题：

| 方法 | 压缩对象 | 核心思路 | 压缩比 | 信息损失 | 额外成本 |
|------|---------|---------|--------|---------|---------|
| **滑动窗口截断** | 对话历史 | 只保留最近 N 轮，丢弃更早的消息 | 高 | 高（丢失早期上下文） | 零 |
| **摘要压缩** | 对话历史 / Episodic Memory | 用 LLM 把多轮对话压缩成一段摘要 | 高（10:1 以上） | 低（语义保留） | 一次 LLM 调用 |
| **实体提取** | 对话历史 | 从对话中提取关键实体和关系，丢弃原文 | 高 | 中（丢失语气、细节） | 一次 LLM 调用 |
| **Token-aware 裁剪** | 单条消息 | 按 token 预算截断工具输出、长回复 | 可控 | 中 | 零 |
| **分层压缩** | 全部记忆 | 近期保留原文，中期摘要化，远期只留实体 | 渐进式 | 渐进式 | 周期性 LLM 调用 |

**方法 1：滑动窗口截断** — 最简单的压缩，直接丢弃最早的对话记录。缺点明显：如果用户在第 3 轮说了一个关键约束，到第 20 轮这个约束就丢了。适合闲聊型场景，不适合长任务。

**方法 2：摘要压缩** — 前面已经详细展示了 `llm_summarize` 的实现。关键技巧是**渐进式摘要**：不是等对话很长了一次性压缩，而是每积累 N 轮就把之前的摘要和新对话合并成一份更新的摘要。

```python
class IncrementalSummarizer:
    """渐进式摘要：每 N 轮对话自动压缩一次"""

    def __init__(self, llm_client, trigger_interval: int = 10):
        self.llm = llm_client
    ...
    def add_message(self, message: dict):
        ...
        if len(self.recent_messages) >= self.trigger_interval:
            self._compress()
    ...
    def _compress(self):
        ...
        self.running_summary = self.llm.complete(prompt)
        self.recent_messages = []

    def get_context(self) -> list[dict]:
        """返回压缩后的上下文：摘要 + 最近未压缩的消息"""
        context = []
...
**方法 3：实体提取** — 比摘要更结构化。从对话中抽取 `(实体, 属性, 值)` 三元组，用结构化数据替代自然语言。压缩比极高，但会丢失语气和上下文细微之处。

```python
ENTITY_EXTRACTION_PROMPT = """Extract key entities and facts from this conversation.
Output JSON: {"entities": [{"name": "...", "type": "...", "attributes": {"key": "value"}}]}

Rules:
- Only extract facts that would be useful in future turns
- Include user preferences, task parameters, intermediate results
- Skip greetings, acknowledgments, filler content"""

def extract_entities(messages: list[dict], llm_client) -> dict:
    """从对话中提取结构化实体"""
...
**方法 4：Token-aware 裁剪** — 前面的 `rule_based_compress` 就是这类方法的典型。更精细的做法是用 tokenizer 精确计算每条消息的 token 数，然后按优先级裁剪：工具输出优先截断，用户消息最后裁剪。

**方法 5：分层压缩** — 实际上是前四种方法的组合策略，按时间衰减逐级压缩：

```python
class TieredCompression:
    """分层压缩：越旧的记忆，压缩粒度越粗"""

    def compress(self, messages: list[dict], current_turn: int) -> list[dict]:
        result = []
        for i, msg in enumerate(messages):
            age = current_turn - i  # 消息"年龄"

            if age <= 5:
                # 近期（5 轮内）：保留原文
                result.append(msg)
            elif age <= 20:
                # 中期（5-20 轮）：截断到 200 token
                result.append(self._truncate(msg, max_tokens=200))
            else:
                # 远期（>20 轮）：只保留实体
                # 由外层的 IncrementalSummarizer 处理
                pass

        return result

    def _truncate(self, msg: dict, max_tokens: int) -> dict:
        content = msg["content"]
        # 简化实现：按字符估算 token
        if len(content) > max_tokens * 3:
            content = content[:max_tokens * 3] + "...[truncated]"
        return {**msg, "content": content}
```

该选哪种方法？简单场景从滑动窗口开始；需要保留语义时上摘要压缩；实体密集型任务（比如客户画像）用实体提取；生产环境通常用分层压缩把这些方法组合起来。本文第 10 节的遗忘机制和这里的压缩方法是互补的：**压缩决定"怎么瘦身"，遗忘决定"该不该丢"**。
...
在四层记忆中，Layer 4 Semantic Memory 的"读取"操作——即如何从大规模知识库中高效检索相关信息——是一个足够深的话题。它涉及 Ingestion、Chunking、Embedding、Hybrid Retrieval、Reranking 等一系列工程决策，值得单独展开。
