...
title: "规划、推理与反思"
pubDate: "2026-01-12"
description: "LLM 的 next-token prediction 天然缺乏前瞻、深度搜索和回溯校验。规划补前瞻，推理补深度，反思补回溯。本文给出 ReAct、Plan-and-Execute、Tree-of-Thought、Hierarchical 四种范式的核心伪代码与适用边界，反思机制的 schema 与终止条件，以及推理时扩展（test-time compute scaling）对 Agent 架构的影响。"
...
上一篇解决了 Agent 如何保留对话、任务状态、经验和领域知识；有了可用上下文，Agent 仍可能沿局部最优路径一直走错。

LLM 的训练目标是 next-token prediction，这种机制天然缺三类外部结构：**前瞻**（生成第一步时不会考虑第五步的后果）、**深度搜索**（默认只沿一条高概率路径往下写）、**回溯校验**（一旦写下就不能主动回到错误发生处修正）。给 LLM 补上这些缺口的工程手段有三种：规划补前瞻，推理补深度，反思补回溯。三者不是"加上模型就有"，是用 Prompt、Schema、终止条件和评估反馈搭出来的外部脚手架。下面展开四种规划范式的伪代码骨架、反思机制的 schema 与终止条件，以及在 o1/o3 这类推理模型出现后，"什么时候该升级到哪一档"的判断信号怎么变。

---

## 1. 为什么 LLM 缺这三种能力

LLM 的训练目标是 next-token prediction——给定前文预测下一个 token。这种机制天然缺三类能力：

| 缺失能力 | 表现 |
|---------|------|
| **前瞻**（Lookahead） | 生成第一步时不会考虑"这个决定在第五步会导致什么后果"——每步选局部最优，但局部最优叠加不等于全局最优 |
| **深度搜索**（Search） | 默认沿一条路径继续生成，不会系统性展开候选、评估、剪枝 |
...
规划产出如果只留在 Prompt 里仍然难以测试和恢复；下一篇把计划进一步固化为 Chaining、Routing 和 Parallelization 三类显式工作流拓扑。
