...
![Agent 推理范式演进](/images/blog/agentic-10/01-paradigm-evolution.svg)
...
![ReAct 循环](/images/blog/agentic-10/02-react-loop.svg)
...
![Plan-and-Execute 架构](/images/blog/agentic-10/03-plan-and-execute.svg)
...
![Tree-of-Thought 搜索树](/images/blog/agentic-10/04-tree-of-thought.svg)
...
![分层规划树 - 递归分解](/images/blog/agentic-10/05-hierarchical-planning.svg)
...
![反思循环的停止条件](/images/blog/agentic-10/06-reflection-stop-condition.svg)
...
![Reflection 策略选择决策树](/images/blog/agentic-10/08-reflection-strategy-decision.svg)

四个问题形成分层筛选：

1. **Q1：错误成本高吗？** 低成本任务（闲聊、简单查询）直接跳过反思，省 token。
2. **Q2：错误可自动检测吗？** 可以就用外部验证（代码跑测试、SQL 查结果），比 LLM Self-Critique 更可靠也更便宜。
3. **Q3：预算够吗？** 够就上结构化反思 + 多轮重试（最多 3 轮），不够就单轮 Self-Critique 兜底。
4. **Q4：延迟敏感？** 是的话最多一轮反思，超时直接返回当前最优结果。

一句话总结：**先判断值不值得反思，再选择反思方式，最后用预算和延迟做硬约束**——这套决策逻辑可以直接写进 Agent 框架的配置层。

---

## 9. Peer-to-Peer 协议设计深化
...
class P2PTerminationChecker:
    ...
    def should_terminate(
        ...
        """检查是否应终止协作，返回 (是否终止, 原因)"""
        # 条件 1：达成共识
        if all(c >= self.config.consensus_threshold for c in agent_confidences.values()):
        ...
        # 条件 3：Token 预算耗尽
        if total_tokens_used >= self.config.token_budget:
            return True, "token_budget_exceeded"

        # 条件 4：观点停滞——连续 N 轮各 Agent 立场不变
        if len(self.round_opinions) >= self.config.stale_rounds:
            recent = self.round_opinions[-self.config.stale_rounds:]
            if all(r == recent[0] for r in recent):
                return True, "stale_discussion"

        return False, ""

    def record_round(self, opinions: dict[str, str]):
        """记录每轮各 Agent 的核心观点，用于停滞检测"""
        self.round_opinions.append(opinions)
```

四个条件形成多层安全网：共识是正常退出，轮数和 token 是硬性保底，停滞检测是"聪明的"提前退出——避免 Agent 反复说同一句话浪费 token。

### 9.3 冲突解决机制

当两个 Agent 无法达成共识时，需要冲突解决机制。三种常见策略：
...
class ConflictResolver:
    ...
    def resolve(
        self,
        strategy: str,
        positions: dict[str, dict],  # {agent_name: {"opinion": str, "confidence": float, "evidence": list}}
    ) -> dict:
        """
        解决冲突，返回最终决策。

        策略：
        - confidence_weighted: 按信心度加权投票（快速、低成本）
        - evidence_based: 比较证据数量和质量（适合有外部数据支撑的场景）
        - mediator: 引入第三方 LLM 做仲裁（最贵但最公平）
        """
        if strategy == "confidence_weighted":
            return self._confidence_weighted(positions)
        elif strategy == "evidence_based":
            return self._evidence_based(positions)
        elif strategy == "mediator":
            return self._mediator(positions)
        else:
            raise ValueError(f"Unknown strategy: {strategy}")

    def _confidence_weighted(self, positions: dict[str, dict]) -> dict:
        """信心度加权：选择加权信心最高的立场"""
        best_agent = max(positions, key=lambda a: positions[a]["confidence"])
        return {
            "method": "confidence_weighted",
            "winner": best_agent,
            "decision": positions[best_agent]["opinion"],
            "confidence": positions[best_agent]["confidence"],
        }

    def _evidence_based(self, positions: dict[str, dict]) -> dict:
        """证据驱动：比较各方的证据数量和多样性"""
        scores = {}
        for agent, pos in positions.items():
            evidence = pos.get("evidence", [])
            # 简单评分：证据数量 × 平均可信度
            score = len(evidence) * pos["confidence"]
            scores[agent] = score
        ...
        return {
            ...
            "winner": best_agent,
            "decision": positions[best_agent]["opinion"],
            "evidence_score": scores[best_agent],
        }

    def _mediator(self, positions: dict[str, dict]) -> dict:
        """仲裁者模式：用独立 LLM 分析双方论点并做出裁决"""
        summary = "\n\n".join(
            f"**{agent}** (信心: {pos['confidence']}):\n观点: {pos['opinion']}\n"
            f"证据: {pos.get('evidence', ['无'])}"
            for agent, pos in positions.items()
        )
        resp = self.client.chat.completions.create(
            model=self.model,
            messages=[
                {"role": "system", "content":
                    "You are a neutral mediator. Analyze both positions objectively.\n"
                    'Return JSON: {"decision": "...", "reasoning": "...", '
                    '"adopted_from": "agent_name"}'},
                {"role": "user", "content": f"请裁决以下分歧：\n\n{summary}"},
            ],
            response_format={"type": "json_object"},
        )
        import json
        result = json.loads(resp.choices[0].message.content)
        result["method"] = "mediator"
        return result
```

选择策略的经验法则：**日常任务用 `confidence_weighted`（零额外成本），有数据支撑的分歧用 `evidence_based`，高价值决策用 `mediator`（多一次 LLM 调用但更公正）。**

### 9.4 完整的 PeerToPeerProtocol 类

将消息格式、终止条件、冲突解决整合到一个完整协议中：
...
class PeerToPeerProtocol:
    ...
    def __init__(
        ...
        self.termination_checker = P2PTerminationChecker(
            TerminationConfig(
                max_rounds=max_rounds,
                consensus_threshold=consensus_threshold,
            )
        )
        self.conflict_resolver = ConflictResolver(openai.OpenAI())
    ...
    async def run(self, topic: str, initial_content: str = "") -> dict:
        ...
        for round_num in range(1, self.max_rounds + 1):
            round_messages: list[P2PMessage] = []

            # 每个 Agent 依次发言（串行，因为后续 Agent 需要看到前面的消息）
            for agent_name, agent in self.agents.items():
                context = self._build_context(agent_name, round_num)
                response = await agent.respond(topic, context)

                msg = P2PMessage(
                    msg_id=str(uuid.uuid4())[:8],
                    sender=agent_name,
                    receivers=[n for n in self.agents if n != agent_name],
                    msg_type=MessageType(response.get("msg_type", "comment")),
            ...
            # 记录本轮观点快照（用于停滞检测）
            opinions = {name: state["opinion"] for name, state in self.agent_states.items()}
            self.termination_checker.record_round(opinions)

            # 检查终止条件
            confidences = {name: state["confidence"] for name, state in self.agent_states.items()}
            should_stop, reason = self.termination_checker.should_terminate(
                round_num, confidences, total_tokens
            )
            if should_stop:
                break
        ...
        if reason == "consensus_reached":
            # 所有人同意，取信心最高者的观点作为最终结果
            best = max(self.agent_states, key=lambda a: self.agent_states[a]["confidence"])
            final_output = self.agent_states[best]["opinion"]
        else:
            # 未达共识，启动冲突解决
            positions = {
                name: {"opinion": state["opinion"], "confidence": state["confidence"], "evidence": []}
                for name, state in self.agent_states.items()
            }
            resolution = self.conflict_resolver.resolve(self.resolution_strategy, positions)
            final_output = resolution["decision"]
    ...
    def _build_context(self, agent_name: str, round_num: int) -> str:
        """为指定 Agent 构建上下文：之前所有消息的摘要"""
        relevant = [m for m in self.message_history if agent_name in m.receivers or m.sender == agent_name]
        if not relevant:
            return "(首轮发言，尚无历史消息)"
        lines = []
        for m in relevant[-10:]:  # 最多保留最近 10 条，防止上下文爆炸
            lines.append(f"[{m.sender} → Round {m.round_number}] ({m.msg_type.value}): {m.content[:200]}")
        return "\n".join(lines)
```

核心设计要点：每轮串行发言（后续 Agent 能看到前面的消息），每轮结束检查终止条件，未达共识时自动触发冲突解决。`_build_context` 只保留最近 10 条消息，防止上下文窗口爆炸。

---

## 10. Single Agent vs Multi-Agent 的量化对比
...
### 10.3 方案 B：Multi-Agent（Supervisor-Worker）
...
### 11.2 SpecializationEvaluator 类
...
![Agent 范式选择矩阵](/images/blog/agentic-10/07-paradigm-selection-matrix.svg)
