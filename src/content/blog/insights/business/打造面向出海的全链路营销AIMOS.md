...
但要把这个终局落地，先要把业务结构搞清楚——客户买的不是抽象的 OS，是一套具体的业务能力组合。

## 二、业务分层：建站、内容运营、营销传播是三件不同的事

**建站、内容运营、营销传播这三件事必须严格分层，混在一起就是灾难。** 两类常见的踩坑反推出这条硬约束：把内容运营当建站交付，丢的是续费与飞轮；把营销传播当内容生产，丢的是流量主线与归因。
...
**ToB 营销的第一性不是内容质量，而是合格线索质量；内容质量是关键手段，但不是终极目的。** 把"内容质量"奉为第一性的产品方案，常常滑向"为生产而生产"的内容工厂；只有把第一性钉在线索质量/成交质量上，内容中台才不会变成另一个 AIGC 工具。
...
**三 ID 不是事后归并、是过程中强制注入**——内容生成时不打 ID、分发时不带 UTM、承接时不绑客户 ID，到环节 6 测评判优时再"找回归属"就晚了。三 ID 是飞轮的血管：必须在每个心跳里就跑通，不能在终点抽血回查。
...
飞轮悬空（成交回流缺失）只是失败模式之一。还有三种容易忽视的"死法"：
...
**业界对照**：HubSpot 在 Marketing Hub 成型之前，先把 form / list / workflow / analytics 抽为共享 service，然后才有 Sales Hub / Service Hub 复用；Salesforce 把 Sales Cloud 的对象模型沉淀为 Platform，才支撑后续 Service / Marketing Cloud。**共享服务是先重构、再扩张的工程动作**——晚做会带来重写成本，太早做会陷入"为平台而平台"。
...
**AIMOS 是契约系统、不是黑盒**——四层架构的设计原则是把每一层都暴露成"可被上下层调用、可被外部 Agent 调用"的清晰契约。这条原则贯穿本章对护城河、形态分工、生态合作的讨论。
...
**最顶层并列挂 MCP Server 不是未来再做、是 day 1 就要做**——因为它决定了"客户的 Agent 反向调用 AIMOS"的可能性。架构上预留这个口子的工程代价极低（一个工具协议适配器），不预留的代价是日后整条交互层要重做。
...
**DTC 形态下五级阶梯并非全归 AIMOS 所有**。建站在 Shopify、CRM 在 Klaviyo——AIMOS 直接拥有的是"内容运营 + 营销传播 + 承接转化"三段，**通过深度集成延伸到建站与 CRM 两端**（API + 嵌入 UI + Webhook）。这是分工：AIMOS 提供智能与协同主线，Shopify / Klaviyo 提供事务基建，两者共同完成五级闭环。两种形态本质是同一套能力按客户既有基建状态做的两种封装——能力栈与垂直数据飞轮共用。
...
**愿景的可信度取决于敢不敢把假设写在台面上、并给出已验证的工程模板。** 前 5 条是飞轮闭合的内部前提（数据、回流、产能、归因、入口质量），后 2 条是外部市场冲击（AI 搜索、主权 AI）。
...
建站作为漏斗入口，可能引来爬不上阶梯的低质客户——做了建站就不续费、不升单。
...
**愿景要有，退路也要有——主动降级远好于守着完整愿景死。** AIMOS 是高风险高复杂度的产品，飞轮可能因为各种现实原因转不起来（数据接入失败、归因不可信、客户不愿意升单），需要预先想好三条退路：
