...
8. **章际桥句与自辩腔扫描（教科书味）**：grep `"下一章|下一节|本章[换看讲]|本节[换看讲]"` 找教材式导读、grep `"这不是.{1,15}，[而]?是|这里[要也]?诚实"` 找 AI 自辩腔。**默认偏好删**：章节标题已经清楚下一节讲什么、不需要"下一章先拆 X"式预告；"这不是 X、而是 Y"直接陈述 Y 即可、不要先否认 X。极少数情况下"X 不是 Y、而是 Z"是真有 reframe 价值（Y 是常见误解），可保留。

**Checklist 不过、文章不能发布**。这 8 项是硬门槛、不是建议。
